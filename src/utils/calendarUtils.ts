...
import { formatDateForCalendar } from '../lib/calendarEventFormat'
...
export function getEventsForDay(events: CalendarEvent[], date: Date) {
  // Compare in Istanbul date space to avoid UTC-offset day-boundary errors.
  // For all-day events (start = "YYYY-MM-DD"), compare date strings directly.
  // For timed events (start = ISO with offset), parse to Date then format in Istanbul.
  const targetDateStr = formatDateForCalendar(date)

  return events.filter(event => {
    const start = event.start
    if (!start) return false
    // All-day: date-only string
    const eventDateStr = start.length === 10 ? start : formatDateForCalendar(new Date(start))
