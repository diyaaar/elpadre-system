...
interface AuthFormProps {
  mode: 'login' | 'signup'
  onSubmit: (email: string, password: string, rememberMe?: boolean) => Promise<{ error: any }>
  onToggleMode?: () => void
}
...
export function AuthForm({ mode, onSubmit, onToggleMode }: AuthFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // In login mode, we might want to pass rememberMe, but since onSubmit signature doesn't take it, 
    // we handles it in LoginPage after onSubmit returns successfully. We'll expose rememberMe to the parent component via another way or update the signature.
    // Changing the signature of onSubmit to include rememberMe:
    const { error } = await onSubmit(email, password, rememberMe)
        ...
        {mode === 'login' && (
          <div className="flex items-center">
            <input
              ...
              className="h-4 w-4 text-primary focus:ring-primary border-background-tertiary rounded bg-background-secondary"
            />
            <label htmlFor="remember_me" className="ml-2 block text-sm text-text-secondary select-none cursor-pointer">
              Beni hatırla (Bu cihazda oturumu sürekli açık tut)
            </label>
          </div>
        )}
      </div>
