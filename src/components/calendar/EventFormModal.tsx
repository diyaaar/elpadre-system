    ...
    const handleAllDayToggle = useCallback(() => {
        setAllDay(prev => {
            ...
            if (next) {
                ...
                // startStr is a "YYYY-MM-DD" string here. new Date("YYYY-MM-DD") parses as
                // UTC midnight, which is wrong in non-UTC browsers. Append T00:00:00 to force
                // local-midnight parsing, then let formatDateTimeForCalendar read Istanbul time.
                const dateStr = startStr ? startStr.slice(0, 10) : formatDateForCalendar(new Date())
