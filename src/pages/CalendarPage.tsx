...
function toRBCEvent(ev: CalendarEvent): RBCEvent {
  ...
  if (isAllDay) {
    // All-day events from Google arrive as "YYYY-MM-DD" strings.
    // new Date("YYYY-MM-DD") is parsed as UTC midnight per spec, which shifts
    // the displayed day by UTC offset in non-UTC browsers.
    // Appending T00:00:00 (no suffix) forces local-midnight parsing, which is
    // what react-big-calendar expects for all-day events.
    const startStr = ev.start?.length === 10 ? ev.start : (ev.start?.slice(0, 10) ?? '')
    start = new Date(`${startStr}T00:00:00`)

    // Google Calendar end dates for all-day events are exclusive (March 26 event → end = March 27).
    // Subtract one calendar day at the string level to get the inclusive end date
    // before handing to RBC. String-level subtraction avoids DST/millisecond edge cases.
    if (ev.end) {
      const endStr = ev.end.length === 10 ? ev.end : ev.end.slice(0, 10)
      // Parse as local midnight, subtract one day, reparse
      const exclusiveEnd = new Date(`${endStr}T00:00:00`)
      exclusiveEnd.setDate(exclusiveEnd.getDate() - 1)
      end = exclusiveEnd < start ? new Date(start) : exclusiveEnd
    } else {
      end = new Date(start)
    }
  } else {
    // Timed events: ISO 8601 strings with offset (e.g. "...+03:00") parse correctly
    // to the right UTC instant. RBC displays them using the browser's local time,
    // which is fine because the localizer handles timezone display.
    start = ev.start ? new Date(ev.start) : new Date()
    end = ev.end ? new Date(ev.end) : new Date(start.getTime() + 60 * 60 * 1000)
  }
