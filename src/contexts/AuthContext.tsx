...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      // Check for persistent login settings
      const rememberMe = localStorage.getItem('remember_me') !== 'false'
      const isActiveSession = sessionStorage.getItem('active_session') === 'true'
      
      // If "remember me" is false AND this is a new browser session, clear the auth state
      if (!rememberMe && !isActiveSession) {
        ...
        // Otherwise, mark this as an active session and proceed
  ...
  const signOut = async () => {
    ...
    // Clear storage flags on explicit sign out
