...
// Intl formatter parts extractor — always reads time in Istanbul, regardless of browser timezone.
function getIstanbulParts(date: Date): Record<string, string> {
  ...
  // Intl hour12:false can produce "24" for midnight; normalize to "00"
  if (map.hour === '24') map.hour = '00'
  return map
}

/**
 * Format a Date to "YYYY-MM-DDTHH:mm:ss" in Istanbul wall-clock time.
 * Always uses Europe/Istanbul via Intl — never depends on browser timezone.
 */
...
/**
 * Format a Date to "YYYY-MM-DD" in Istanbul wall-clock date.
 * Always uses Europe/Istanbul via Intl — never depends on browser timezone.
 */
